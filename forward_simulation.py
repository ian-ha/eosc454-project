...
class ForwardSimulation:
    ...
    def __init__(self, config_yaml, z_func=None):
        ...
        self.x_survey, self.y_survey, self.z_survey = self.define_survey_locations(self.survey_x_spacing, self.survey_x_bounds, self.survey_y_spacing, self.survey_y_bounds, self.survey_z_height)
        inducing_field_params = self.config.get("inducing_field", None)
        self.survey = self.define_survey(self.x_survey, self.y_survey, self.z_survey, components=survey_params.get("components", "tmi"), inclination=inducing_field_params.get("inclination", 90), declination=inducing_field_params.get("declination", 0), amplitude=inducing_field_params.get("amplitude", 50000))
        mesh_params = self.config.get("mesh", None)
        self.mesh = self.define_mesh(mesh_params.get("dx", None), mesh_params.get("dy", None), mesh_params.get("dz", None), mesh_params.get("x_length", None), mesh_params.get("y_length", None), mesh_params.get("z_length", None))
        self.active_cells = self.define_active_cells(self.mesh)
        model_params = self.config.get("model", None)
        self.model = self.define_model(self.mesh, self.active_cells, background_susceptibility=model_params.get("background_susceptibility", 0.5), curie_depth=model_params.get("curie_depth", 300), curie_susceptibility=model_params.get("curie_susceptibility", 0.05))
    def generate_topography(self):
        """Generate the topography of the simulation domain."""
        [x_topo, y_topo] = np.meshgrid(np.linspace(-self.x_bounds[0], self.x_bounds[1], self.n_cells[0]), np.linspace(-self.y_bounds[0], self.y_bounds[1], self.n_cells[1]))
    ...
    def define_survey_locations(self, x_spacing, x_bounds, y_spacing, y_bounds, z_height):
        ...
        z_survey = np.full((num_y, num_x), z_height)  # Survey points at a constant height
        return x_survey, y_survey, z_survey
    
    def define_survey(self, x_survey, y_survey, z_survey, components="tmi", inclination=90, declination=0, amplitude=50000):
    ...
    def define_model(self, mesh, active, background_susceptibility=0.5, curie_depth=300, curie_susceptibility=0.05):
        ...
        model[active & (z_cell_centers < self.z_topo.max() - curie_depth)] = curie_susceptibility
        return model
